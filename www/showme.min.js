...
function binl2b64(a){for(var f="",b=0;b<a.length*4;b+=3)for(var c=(a[b>>2]>>8*(b%4)&255)<<16|(a[b+1>>2]>>8*((b+1)%4)&255)<<8|a[b+2>>2]>>8*((b+2)%4)&255,e=0;e<4;e++)f+=b*8+e*6>a.length*32?b64pad:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(c>>6*(3-e)&63);return f};var map=null,map_extent=null,documents={},extents={},properties={};
function sm_init(){var a=navigator.userAgent.toLowerCase().indexOf("chrome")>-1;if(typeof FileReader=="undefined"||a){a=document.getElementById("load_file");a.setAttribute("type","text");a.setAttribute("value","disabled (by your browser)");a.setAttribute("disabled","disabled");a.setAttribute("style","font-style:italic;background-color:transparent;border:none;")}a=org.polymaps.svg("svg");a=document.getElementById("map").appendChild(a);map=org.polymaps.map();map.container(a);map.zoomRange([1,17]);a=
org.polymaps.image();a.url("http://spaceclaw.stamen.com/toner/{Z}/{X}/{Y}.png");map.add(a);a=org.polymaps.interact();map.add(a);a=org.polymaps.compass();a.zoom("small");a.pan("none");map.add(a);sm_extents_jumpto()};function sm_form_handler(){var a=document.getElementById("load_uri"),f=document.getElementById("load_file"),b=0;if(a.value){b=1;if(a.value.indexOf("http")==0){sm_load_uri(a.value);a.value=""}else alert("Invalid URL!")}if(f.files.length){b=1;sm_load_files(f.files);f.value=""}b==0&&alert("Unable to find anything to show!")}function sm_form_toggle(a){document.getElementById("load_header").style.display=a?"block":"none";document.getElementById("load_form").style.display=a?"none":"block"};function sm_load_uri(a){if(!extents[a]){var f=org.polymaps.geoJson();f.url(a);var b=sm_onload(a);f.on("load",b);map.add(f);documents[a]=f;sm_extents_set_onload(a);sm_form_toggle("close")}}function sm_load_features(a,f){f||(f=hex_md5(JSON.stringify(a)));if(!extents[f]){var b=org.polymaps.geoJson();b.features(a);var c=sm_onload(f);b.on("load",c);map.add(b);documents[f]=b;sm_extents_set_onload(f);sm_form_toggle("close")}}function sm_load_files(a){for(var f=a.length,b=0;b<f;b++)sm_load_file(a[b])}
