...
function sm_init(){has_filereader=typeof FileReader!=="undefined"?1:0;if(navigator.userAgent.toLowerCase().indexOf("chrome")!==-1)has_filereader=0;if(!has_filereader){var a=document.getElementById("load_file");a.setAttribute("type","text");a.setAttribute("value","disabled (by your browser)");a.setAttribute("disabled","disabled");a.setAttribute("style","font-style:italic;background-color:transparent;border:none;")}a=org.polymaps.svg("svg");a=document.getElementById("map").appendChild(a);map=org.polymaps.map();
map.container(a);map.zoomRange([1,17]);a=org.polymaps.image();a.url("http://spaceclaw.stamen.com/toner/{Z}/{X}/{Y}.png");map.add(a);a=org.polymaps.interact();map.add(a);a=org.polymaps.compass();a.zoom("small");a.pan("none");map.add(a);sm_extents_jumpto()};function sm_form_handler(){var a=document.getElementById("load_uri"),f=document.getElementById("load_file"),b=0;if(a.value){b=1;if(a.value.indexOf("http")==0){sm_load_uri(a.value);a.value=""}else alert("Invalid URL!")}if(has_filereader&&f.files.length){b=1;sm_load_files(f.files);f.value=""}b==0&&alert("Unable to find anything to show!")}
function sm_form_toggle(a){document.getElementById("load_header").style.display=a?"block":"none";document.getElementById("load_form").style.display=a?"none":"block"};function sm_load_uri(a){if(!extents[a]){var f=org.polymaps.geoJson();f.url(a);var b=sm_onload(a);f.on("load",b);map.add(f);documents[a]=f;sm_extents_set_onload(a);sm_form_toggle("close")}}function sm_load_features(a,f){f||(f=hex_md5(JSON.stringify(a)));if(!extents[f]){var b=org.polymaps.geoJson();b.features(a);var c=sm_onload(f);b.on("load",c);map.add(b);documents[f]=b;sm_extents_set_onload(f);sm_form_toggle("close")}}function sm_load_files(a){for(var f=a.length,b=0;b<f;b++)sm_load_file(a[b])}
function sm_load_file(a){var f;try{f=new FileReader}catch(b){sm_load_uri(a.name);return}f.onloadend=function(c){c=c.target;if(c.error)alert("your browser failed to load that document, with the following error: "+c.error.code);else{try{geojson=JSON.parse(c.result)}catch(e){alert("failed to parse your file ("+a.name+"), the error was: "+e);return}sm_load_features(geojson.features,a.name)}};f.readAsBinaryString(a)};function sm_onload(a){return function(f){sm_onload_json(f,a)}}
...
;function sm_extents_set_onload(a){if(a)if(extents[a]){sm_extents_set();sm_documents_list()}else{extent_timeout&&clearTimeout(extent_timeout);extent_timeout=setTimeout(function(){sm_extents_set_onload(a)},200)}}
function sm_extents_set(){var a=null,f=null,b=null,c=null;for(uid in extents){bbox=extents[uid];a=a?Math.min(a,bbox[0].lat):bbox[0].lat;f=f?Math.min(f,bbox[0].lon):bbox[0].lon;b=b?Math.max(b,bbox[1].lat):bbox[1].lat;c=c?Math.max(c,bbox[1].lon):bbox[1].lon}map_extent=[{lat:a,lon:f},{lat:b,lon:c}];sm_extents_jumpto()}
